export function newShip(length) {
  return {
    ...
    isSunk: function () {
      return this.hitAreas.length == this.length
        ? (this.sunk = true)
        : (this.sunk = false);
    },
  ...
  return {
    ...
    checkIfFit(ship, coordX, coordY, orientation) {
      ...
      if (orientation == "vertical") {
        ...
        for (let i = 0; i < ship.length; i++) {
          if (typeof this.board[coordX + i + coordY * 10] == "object")
            return false;
        }
      }
      return true;
    ...
    removeShip: function (ship) {
      for (let i = 0; i < this.board.board.length; i++) {
        if (this.board.board[i] != 0) {
          if (
            this.board.board[i].start[0] === ship.start[0] &&
            this.board.board[i].start[1] === ship.start[1]
          )
    ...
    randomizeBoard: function () {
      while (this.ships.length > 0) {
        ...
        for (let i = 0; i < currentShip[1]; i++) {
          ...
          if (
            this.board.checkIfFit(currentShip[0], coordX, coordY, result) ==
            true
          ) {
